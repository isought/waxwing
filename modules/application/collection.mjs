...
export async function buildCollection(input, output) {
  ...
  function resolve(target) {
    ...
    if(target.kind!==undefined||target.ref!==undefined) {
      if(!['graph','workflow','document','node','edge','step','block',...(entry.layout.model.diagramType==='relational'?['record','model','table','column','constraint','index','association','source']:[])].includes(target.kind))throw new Error('Invalid collection target kind.');
    ...
    if(!html||fragment&&entry.layout.model.diagramType!=='relational'&&!html.includes(`id="${decodeURIComponent(fragment)}"`))throw new Error(`Collection target does not exist: ${target.site}/${local}`);
  ...
  files.set('index.html',shell('index.html',config.title,`<p class="purpose">${esc(config.description??'Explore the system, then follow the scenarios that matter.')}</p><section class="start-panel"><div><div class="eyebrow">START HERE</div><h2>${esc(start.entry.title)}</h2><p>${esc(start.entry.description)}</p><a class="primary-link" href="${esc(start.path)}">Open explanation →</a></div><div><h3>One place to explore</h3><p>${entries.length} explanations, each with its own scope and evidence.</p><a href="search.html">Search across all content →</a></div></section><div class="collection-grid">${entries.map((e,i)=>`<section class="collection-card"><div class="eyebrow">${String(i+1).padStart(2,'0')} · ${e.layout.model.diagramType==='sequence'?'SEQUENCE':e.layout.model.diagramType==='relational'?'RELATIONAL':'ARCHITECTURE'}</div><h2><a href="sites/${e.id}/index.html">${esc(e.title)}</a></h2><p>${esc(e.description)}</p><p class="result-context">${esc(e.layout.model.scope.environment)} · ${esc(e.layout.model.scope.snapshot??e.layout.model.scope.timeframe)}</p><details><summary>Views & documents · ${sitePages(e.layout.model).length}</summary><ol>${sitePages(e.layout.model).map(p=>`<li><a href="sites/${e.id}/${p.path}">${esc(p.title)}</a></li>`).join('')}</ol></details></section>`).join('')}</div><p>Reading order and links organize these explanations. They do not merge component identities or assert architectural relationships.</p>`));
