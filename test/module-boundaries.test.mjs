  ...
  for (const [owner, allowed] of Object.entries(dependencies)) for (const file of files(path.join(modules, owner))) {
    ...
    for (const specifier of imports) {
      ...
      if (!specifier.startsWith('.')) {
        if (!['ajv', 'markdown-it', ...(owner === 'presentation' ? ['elkjs'] : []), ...(owner === 'analysis' ? ['typescript', 'web-tree-sitter', 'tree-sitter-wasm'] : [])].some(name => specifier === name || specifier.startsWith(name + '/'))) {
